...
#define ctMax(a,b) (ctMax_impl<(a),(b)>::value)
...
HLS_INLINE fp_flopoco<wER,wFR> mul(const fp_flopoco<wEX,wFX> &x, const fp_flopoco<wEY,wFY> &y)
{
...
template<int wER,int wFR,int wEX,int wFX,int wEY,int wFY>
HLS_INLINE fp_flopoco<wER,wFR> mul_v2(const fp_flopoco<wEX,wFX> &x, const fp_flopoco<wEY,wFY> &y)
{
	static const int DEBUG=0;
    ...
    if (1+wFR >= wFX+wFY+2) {
        ...
        fw_uint<2> excPostNorm = tab[expSigPostRoundB.to_int()];
...
template<int wER,int wFR,int wEX,int wFX,int wEY,int wFY>
HLS_INLINE fp_flopoco<wER,wFR> mul_v3(const fp_flopoco<wEX,wFX> &x, const fp_flopoco<wEY,wFY> &y)
{
	static const int DEBUG=0;
    ...
    //check if rounding is needed (we only support the case it is)
    assert( (1+wFR < wFX+wFY+2) );
	...
	fw_uint<2+wER+wFR> expSigPostRoundNo = expSig;
	fw_uint<2+wER+wFR> expSigPostRoundYes = expSig + fw_uint<2+wER+wFR>(1);
	...
	fw_uint<2+wER+wFR> expSigPostRound = round.to_bool() ? expSigPostRoundYes : expSigPostRoundNo;
