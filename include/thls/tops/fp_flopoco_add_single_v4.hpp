#ifndef thls_fp_flopoco_add_v4_hpp
#define thls_fp_flopoco_add_v4_hpp
...
THLS_INLINE fp_flopoco<wER,wFR> add_single(const fp_flopoco<wEX,wFX> &xPre, const fp_flopoco<wEY,wFY> &yPre, int DEBUG=0)
{
    ...
    */
    ...
    unsigned shiftValI=shiftVal.to_int();
    assert(shiftValI<wF+4);
    fw_uint<2*wF+4> shiftedFracY = preFracY >> shiftValI;
...
// 	vhdl << tab << declare("expPart",wE+2) << " <= (" << zg(wE+2-lzocs->getCountWidth(),0) <<" & nZerosNew) - 1;"<<endl;
    ...
    /*auto addToRoundBit = select(
        ...
        og<1>());*/
    auto addToRoundBit = concat(lsb,grd,rnd,stk) ==cg<4>(0b0100);
    //round
...
// 		vhdl<<tab<<declare("RoundedExpFrac",wE+2+wF+1)<<"<= expFrac + addToRoundBit;"<<endl;
    ...
    auto excR = select( concat(eqdiffsign,EffSub,excRt2)==cg<4>(0b1101), zg<2>(), excRt2);
    ...
    auto signR2 = (concat(eqdiffsign,EffSub,excR)!=cg<4>(0b1100)) & signR;
