...
std::string fp_flopoco<ExpBits,FracBits>::str() const
{
    ...
    if( (flags==0b00).to_bool() ){
        ...
        acc<<" expnt="<<expnt<<"="<<(expnt.to_int()-128);
        
        acc<<" frac="<<frac<<"= (approx)"<<(ldexp(frac.to_int()+ldexp(1,FracBits),-FracBits));
...
namespace detail
{
    ...
    struct promote_exp_impl
    {
        static fw_uint<DstE> go(const fw_uint<SrcE> &e)
        {
            ...
            static const int DstBias=(1<<(SrcE-1))-1;
            
            static_assert(SrcBias >= DstBias, "Assumptions violated"); 
            
            return e+(DstBias-SrcBias);
        }
    };
